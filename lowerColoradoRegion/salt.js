/* 
  ...
  the subwatersheds in the Salt basin using Landsat annual 
...
// Salt basin.
var salt = ee.FeatureCollection("USGS/WBD/2017/HUC06").filter(ee.Filter.eq('huc6', '150601'));

// The 220 subwatersheds within the Salt basin.
var subwatersheds = ee.FeatureCollection("USGS/WBD/2017/HUC12")
  .filter(ee.Filter.and(
    ee.Filter.gte('huc12','150601000000'),
    ee.Filter.lt('huc12','150602000000')
  )
);

// Loop through years.
for (var i = 2020; i >= 1985; i--){
  var image = utils.makeAnnual(salt, i);
  ...
  Export.table.toCloudStorage({
    collection: data, 
    description: 'salt_export_' + iStr,
    bucket: 'YOUR-GOOGLE-CLOUD-STORAGE-BUCKET',
    fileNamePrefix: 'salt_subwatersheds_' + iStr,
    fileFormat: 'CSV',
  });
}